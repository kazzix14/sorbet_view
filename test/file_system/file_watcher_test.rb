...
class FileWatcherTest < Minitest::Test
  ...
  def test_ignores_non_erb_files
    Dir.mktmpdir do |dir|
      views_dir = File.join(dir, 'app', 'views')
      FileUtils.mkdir_p(views_dir)
      ...
      # Create a non-ERB file
      File.write(File.join(views_dir, 'test.rb'), 'puts "hello"')
      sleep 0.5

      watcher.stop

      assert changes.empty?, 'Should not detect non-ERB files'
    end
  end
end
